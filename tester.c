...
int test_cache_fifo_insert();
int test_cache_fifo_lookup();
...
int main(int argc, char *argv[])
{
  ...
  score += test_cache_fifo_insert();
  score += test_cache_fifo_lookup();
...
/* Testing FIFO in the case of insert-only workload. We insert 4 entries to a
 * cache of size 3. The last inserted entry should evict the first inserted
 * entry because the first entry */
int test_cache_fifo_insert() {
  printf("running %s: ", __func__);
...
/* Testing FIFO in the case of a workload with inserts and lookups. We insert 3
...
int test_cache_fifo_lookup() {
  ...
  if (cache_lookup(3, 5, out) != -1) {
    printf("failed: the entry 3, 5 should have been evicted but it was not.\n");
    goto out;
  }
